  ...
  return (
    <div className="relative flex items-center">
      ...
      <input
        ...
        className={cn(
          'focus-ring h-8 w-full rounded-[5px] border border-line bg-surface pl-8 pr-8',
      ...
      {value ? (
        <button
          ...
          onClick={() => onChange('')}
          className="absolute right-2 flex h-4 w-4 items-center justify-center rounded-[3px] text-label-3 hover:bg-surface-2 hover:text-label"
        >
...
export function SegmentedControl({ options, value, onChange, className }) {
  return (
    <div
      role="tablist"
      className={cn(
        'inline-flex w-full overflow-hidden rounded-[5px] border border-line bg-surface',
        className,
      )}
    >
      {options.map((opt, i) => {
...
export function Stepper({ value, onChange, min = 0, max = 9999, step = 1, unit }) {
  ...
  return (
    <div className="inline-flex items-stretch overflow-hidden rounded-[5px] border border-line">
      <button
        type="button"
        aria-label="Decrease"
...
export function Pill({ tone = 'quiet', children, className, icon: Icon }) {
  return (
    <span
      className={cn(
        'inline-flex shrink-0 items-center gap-1 rounded-[3px] px-1.5 py-[1px]',
...
export function Toggle({ checked, onChange, label }) {
  return (
    <button
      ...
      className={cn(
        'focus-ring relative h-[20px] w-[34px] shrink-0 rounded-[3px] border transition-colors duration-100',
...
export function EmptyState({ icon: Icon, title, body, action }) {
  return (
    <div className="flex flex-col items-center justify-center px-6 py-14 text-center">
      {Icon ? (
        <span className="mb-3 flex h-10 w-10 items-center justify-center rounded-[6px] border border-line bg-surface text-label-3">
